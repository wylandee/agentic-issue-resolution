"""Run the upstream prerequisite Juice Shop fixture."""
...
_DEFAULT_ISSUES = Path(__file__).resolve().parent / "baseline_issues_upstream_prerequisites.jsonl"
_DEFAULT_SUPPRESSIONS = Path(__file__).resolve().parent / "suppressions.xml"
_DEFAULT_OUTPUT = (
    _PROJECT_ROOT / "data" / "trajectories" / "juice-shop-upstream-prerequisites-result.json"
)
_DEFAULT_PATCH = _PROJECT_ROOT / "data" / "trajectories" / "juice-shop-upstream-prerequisites.patch"
...
_EXPECTED_ISSUE_COUNT = 20


def load_upstream_prerequisites_issues(
    path: Path = _DEFAULT_ISSUES,
) -> list[VulnerabilityIssue]:
    """Load and validate the upstream-prerequisites canonical JSONL fixture.
    ...
    if len(issues) != _EXPECTED_ISSUE_COUNT or packages != _TARGET_PACKAGES:
        raise ValueError(
            "Upstream-prerequisites fixture must contain exactly 20 findings for "
...
def main() -> int:
    ...
    issues = load_upstream_prerequisites_issues(issues_path)
    copied_suppressions = copy_suppressions_to_repo(suppressions_path, repo_root)
    logger.info(
        "Starting upstream_prerequisites fixture with %d findings; copied suppressions to %s.",
        len(issues),
        copied_suppressions,
    )
