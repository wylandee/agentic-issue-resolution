...
# These tools mutate or validate the package-manifest transaction owned by the
# update worker.  They must not be executed in parallel because the tool
# contract requires ``modify -> validate`` for one package before another
# package can be touched.
...
def _manifest_call_deferred_instruction(deferred_tool_names: Sequence[str]) -> str:
    """Tell the worker to continue a batch after a serialized tool call.

    OpenAI-style tool calling can return several tool calls in one assistant
    message.  The worker cannot observe the result of the first manifest
    operation until the next model turn, so later manifest operations from the
    same message must be acknowledged but not executed.

    Args:
        deferred_tool_names: Names of manifest calls that were acknowledged but
            not executed from the current assistant message.

    Returns:
        A human message that causes the next model turn to resume the batch.
    """
...
def run_bounded_subagent_loop(
    ...
    # Ask providers that support the option not to emit parallel tool calls.
    # The runtime barrier below remains authoritative for providers that ignore
    # this hint or for models that still return multiple calls in one message.
        ...
        for tool_call in tool_calls:
            ...
            if call_signature in recovery_signatures:
                ...
                if tool_name in _SERIAL_MANIFEST_TOOL_NAMES and manifest_call_seen:
                    ...
                    tool_message = ToolMessage(
                        content=(
                            "DEFERRED: manifest tool calls are serialized. "
                            "This call was not executed; continue it in the next "
                            "assistant turn after the current package operation "
                            "has been observed."
                        ),
                        tool_call_id=tool_call_id,
                        name=tool_name,
