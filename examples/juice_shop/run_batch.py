"""Run the remediation engine multiple times against Juice Shop with sampled package batches."""
...
logger = logging.getLogger("juice_shop_batch_runner")
...
def parse_package_name_from_url(url_pattern: str) -> str | None:
    ...
    Supports URL-encoded scoped npm packages (%40angular%2Fcommon),
    standard npm/javascript packages, and maven coordinates.

    Args:
        url_pattern: Regex pattern such as '^pkg:npm/%40angular%2Fcommon@.*$'.

    Returns:
        Unescaped package name, or None if pattern does not match.
    """
    decoded = unquote(url_pattern)
    match_npm = re.search(r"pkg:(?:npm|javascript)/(.+?)@", decoded)
    if match_npm:
        raw_name = match_npm.group(1)
        return raw_name.replace(r"\-", "-").replace(r"\.", ".")
    match_mvn = re.search(r"pkg:maven/([^/]+)/([^@]+)@", decoded)
    if match_mvn:
        group, art = match_mvn.group(1), match_mvn.group(2)
        return f"{group}:{art}"
    return None


def extract_ecosystem_from_url(url_pattern: str) -> str:
    """Extract ecosystem from a packageUrl pattern (npm, javascript, or maven).

    Args:
        url_pattern: Regex pattern such as '^pkg:npm/%40angular%2Fcommon@.*$'.

    Returns:
        Ecosystem string ('npm', 'javascript', or 'maven').
    """
    decoded = unquote(url_pattern)
    match = re.search(r"pkg:(npm|javascript|maven)/", decoded)
    ...
    def _replace_block(match: re.Match[str]) -> str:
        ...
        if pkg_name and pkg_name in selected_packages:
            ...
            return (
                "<!--\n"
                "    <suppress>\n"
                f"        <notes>Keep {pkg_name} as a selected Juice Shop fixture</notes>\n"
...
def run_batch_iteration(
    *,
    ...
    Args:
        ...
        repo_root: Path to Juice Shop clone.
    ...
    # 4. Copy suppressions.xml to repo root
    copy_suppressions_to_repo(suppressions_xml_path, repo_root)

    timestamp = datetime.now(UTC).isoformat()
    result_path = output_dir / f"juice-shop-run-{iteration:02d}-result.json"
    patch_path = output_dir / f"juice-shop-run-{iteration:02d}.patch"
...
def run_batch(
    *,
    ...
    summary_path = output_dir / "juice-shop-batch-runs-summary.json"
    summary_payload = {
...
def main() -> int:
    """CLI entrypoint for Juice Shop batch remediation runner."""
    load_dotenv()
    ...
    parser.add_argument(
        ...
        help="Path to Juice Shop clone (default: data/clones/juice-shop)",
    )
    parser.add_argument(
        "--baseline",
        type=Path,
        default=_DEFAULT_BASELINE,
        help="Path to baseline issues fixture (default: examples/juice_shop/fixtures/baseline_issues.jsonl)",
    )
    parser.add_argument(
        "--suppressed-issues",
        type=Path,
