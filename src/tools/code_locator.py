"""
code_locator.py — SAST finding localization using tree-sitter.
...
``locate_sast(issue, repo_root) -> LocalizedIssue``
    ...
    - ``enclosing_symbol`` / ``enclosing_node_type``  — the nearest enclosing
      function, method, class, or arrow function (via tree-sitter AST walk).
    - ``sink_expression`` — the call-expression at the finding line.
    - ``imports`` — top-level import / require statements from the file.
    - ``snippet`` — a ≤ 30-line code excerpt centred on the finding.
    - ``localization_confidence`` — 0.0–1.0 score based on what was found.
    - ``data_flow_hints`` — simple taint-style hints derived from the snippet.
...
* No network calls.
* No file writes.
* Graceful degradation at every step — a ``LocalizedIssue`` is always returned,
...
def _score_confidence(
    *,
    ...
    """Compute a 0–1 localization confidence score.

    Weights:
    - file_readable: prerequisite — if False, score = 0.0
...
def locate_sast(
    ...
    Returns:
        A ``LocalizedIssue`` — always.  On any failure the result will have
    ...
    if issue.issue_type != IssueType.SAST:
        log.warning(
            "locate_sast called with non-SAST issue %s (type=%s) — returning stub.",
            issue.id,
            issue.issue_type,
        )
