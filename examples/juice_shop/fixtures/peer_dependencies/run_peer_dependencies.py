"""Run the Hono peer-dependency Juice Shop fixture."""
...
_DEFAULT_ISSUES = Path(__file__).resolve().parent / "baseline_issues_peer_dependencies.jsonl"
_DEFAULT_SUPPRESSIONS = Path(__file__).resolve().parent / "suppressions.xml"
_DEFAULT_OUTPUT = (
    _PROJECT_ROOT / "data" / "trajectories" / "juice-shop-peer-dependencies-result.json"
)
_DEFAULT_PATCH = _PROJECT_ROOT / "data" / "trajectories" / "juice-shop-peer-dependencies.patch"
...
def load_peer_dependencies_issues(
    path: Path = _DEFAULT_ISSUES,
) -> list[VulnerabilityIssue]:
    """Load and validate the peer-dependencies canonical JSONL fixture.
    ...
    if len(issues) != _EXPECTED_ISSUE_COUNT or packages != _TARGET_PACKAGES:
        raise ValueError(
            "Peer-dependencies fixture must contain exactly 9 findings for "
...
def main() -> int:
    ...
    issues = load_peer_dependencies_issues(issues_path)
    copied_suppressions = copy_suppressions_to_repo(suppressions_path, repo_root)
    logger.info(
        "Starting peer_dependencies fixture with %d findings; copied suppressions to %s.",
        len(issues),
        copied_suppressions,
    )
