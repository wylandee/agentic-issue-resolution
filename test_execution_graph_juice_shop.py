...
# Set logging to INFO to watch LangGraph's node transitions
...
def test_react_phase5_graph():
    logger.info("==================================================")
    logger.info("🤖 STARTING PHASE 5 REACT LANGGRAPH TEST (MULTI-CVE)")
    logger.info("==================================================")
    ...
    if retries > 0:
        logger.info(f"   ↳ 🔄 The Agent had to self-correct {retries} time(s).")
    ...
    # =========================================================
    # 🕵️‍♂️ NEW: INSPECT THE LLM's ReAct CONVERSATION
    # =========================================================
    ...
    if messages:
        ...
        for msg in messages:
            ...
            if isinstance(msg, AIMessage):
                if msg.content:
                    logger.info(f"   🤖 LLM: {msg.content}")
                for tool_call in getattr(msg, "tool_calls", []):
                    logger.info(f"   🛠️  CALLING TOOL: {tool_call['name']}")
