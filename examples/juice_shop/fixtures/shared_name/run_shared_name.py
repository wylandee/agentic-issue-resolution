"""Run the shared-name Angular package-cluster Juice Shop fixture."""
...
_DEFAULT_ISSUES = Path(__file__).resolve().parent / "baseline_issues_shared_name.jsonl"
_DEFAULT_SUPPRESSIONS = Path(__file__).resolve().parent / "suppressions.xml"
_DEFAULT_OUTPUT = _PROJECT_ROOT / "data" / "trajectories" / "juice-shop-shared-name-result.json"
_DEFAULT_PATCH = _PROJECT_ROOT / "data" / "trajectories" / "juice-shop-shared-name.patch"
...
_EXPECTED_ISSUE_COUNT = 9


def load_shared_name_issues(
    path: Path = _DEFAULT_ISSUES,
) -> list[VulnerabilityIssue]:
    """Load and validate the Angular-only canonical JSONL fixture.

    Args:
        path: JSONL file containing the selected baseline findings.

    Returns:
        The nine canonical Angular findings in fixture order.
    ...
    if len(issues) != _EXPECTED_ISSUE_COUNT or packages != _TARGET_PACKAGES:
        raise ValueError(
            "Shared-name fixture must contain exactly nine findings for "
...
def copy_suppressions_to_repo(
    ...
    Side Effects:
        Overwrites ``repo_root / "suppressions.xml"``.
    """
...
def main() -> int:
    ...
    issues = load_shared_name_issues(issues_path)
    copied_suppressions = copy_suppressions_to_repo(suppressions_path, repo_root)
    logger.info(
        "Starting shared-name Angular fixture with %d findings; copied suppressions to %s.",
        len(issues),
        copied_suppressions,
    )
