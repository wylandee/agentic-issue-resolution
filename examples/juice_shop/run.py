...
def main() -> int:
    ...
    parser.add_argument("--repo", type=Path, default=Path(os.environ.get("TEST_REPO_ROOT", "data/clones/juice-shop")))
    parser.add_argument("--issues", type=Path, default=Path(__file__).parent / "fixtures" / "baseline_issues.jsonl")
    parser.add_argument("--output", type=Path, default=Path("data/trajectories/juice-shop-result.json"))
    parser.add_argument("--patch-out", type=Path, default=Path("data/trajectories/juice-shop.patch"))
    ...
    if not args.repo.is_dir():
        ...
        return 2
    result = run_remediation(RemediationRequest(repo_root=args.repo, issues=_load_issues(args.issues)))
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(result.model_dump(exclude={"raw_state"}), indent=2, default=str) + "\n", encoding="utf-8")
